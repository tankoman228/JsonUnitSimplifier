...
"random" - либо значение формата [-1.5, 3] (случайное число от и до), либо строка случайного выражение (читайте далее)
...
Спецсимволы можно экранировать обратным слешем (в JSON \ заносится как \\, так как он - тоже спецсимвол): 
'\\[' и '\\]' = '[' и ']', '\\{' и '\\}' = '{' и '}'
