...
Подключите файл(ы) библиотеки .dll к проекту, добавив в зависимости вашего проекта для тестирования


Далее в этой главе описывается пример написания юнит-теста при помощи JsonUnitSimplifier.

// код на C#, описывающий тест
TestByJSON.TestObject<YourTestedObjectClass>(contents_of_json_file, o => {
    // Ваша доп. логика для каждого YourTestedObjectClass o из будущего датасета        
});

Далее создайте JSON файл для описания модульного теста. 
В самом простом случае файл начинается следующим образом:
...
"mode" - режим создания объектов датасета:
	...
	- если "mode" = "fields", объект создаётся пустой, задаются поля
	
"combination_mode" - режим комбинирования правил создания датасета:
	- если "combination_mode" = "simple", датасет будет составлен из минимального возможного 
	числа комбинаций правил так, чтобы каждое правило приняло как минимум одно возможное значение
...
Так если у нас 3 правила, которые предполагают 2, 4, 3 возможных значений поля (аргумента), тогда в режиме: 
	при "combination_mode" = "simple" будет 4 объекта, возможные варианты значений 1 и 3 правил зацикливаются
...
При "mode" = "constructor" имя поля не обязательно, но порядок правил должен соответствовать 
аргументам и типам аргументов конструктора


Содержимое правил имеет следующие компоненты:

"field_type" - тип поля конструктора, необходим для жёсткого приведения типов, особенно int и float
...
Чтобы использовать функцию для генерации датасета, в коде необходимо прописать нечто наподобие:
...
Не забывайте, класс возвращаемого object важен, особенно для конструктора.
...
Но даже при "combination_mode" = "all-to-all" число комбинаций от функции по умолчанию 1, чтобы
...
Так если указать первый вариант как единственное правило, будет всего 1 объект: f(0),
но если указать "function_calls": 6, тогда это будут f(0), f(1), f(2), f(3), f(4), f(5).

Если, к примеру, будет 2-е правило с n комбинациями, тогда i будет от 0 до 6n при "all-to-all", при "simple" max(6, n)
...
- "function": соответствие значению возврата функции(int i), i - порядковый номер в датасете. 
	См. атрибут [GenerateFunction("имя")] или GenerateFunctions.AddFunc(string key, Func<int, object> func)

Если проверка провалена, будет вызвано исключение с соответствующим сообщением.


Параметры проверок, отвечающие за получаемое значение:

"function" - вызов функции, если "args" одномерный массив, тогда для каждого объекта 1 набор аргументов, 
